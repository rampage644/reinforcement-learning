...
def reinforce(env, estimator_policy, estimator_value, num_episodes, discount_factor=1.0):
    ...
    for i_episode in range(num_episodes):
        ...
        for t in itertools.count():
            ...
            episode.append(Transition(state, action, reward, n_state, done))
        ...
        for t, transition in enumerate(episode):
            ...
            Gt = sum(discount_factor ** i * t.reward for i, t in enumerate(episode[t:]))
            # Update our value estimator
            estimator_value.update(transition.state, Gt)
            # Calculate baseline/advantage
            advantage = Gt - estimator_value.predict(transition.state)
...
with tf.Session() as sess:
    ...
    stats = reinforce(env, policy_estimator, value_estimator, 10000, discount_factor=1.0)
